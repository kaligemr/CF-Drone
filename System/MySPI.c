...
void MySPI_ICM42688_Init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;	//波特率分频，选择4分频
...
{
	MySPI_ICM42688_W_SS(0);				//拉低SS，开始时序
}
...
{
	MySPI_ICM42688_W_SS(1);				//拉高SS，终止时序
}
...
void MySPI_SPL06_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);					//将PA12引脚初始化为推挽输出
...
//	SPI_Cmd(SPI2, DISABLE);             //必须先禁能,才能改变MODE
	...
	SPI_InitStructure.SPI_DataSize = SPI_DataSize_8b;		//数据宽度，选择为8位
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;		//先行位，选择高位先行
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;	//波特率分频，选择8分频,APB1为32Mhz
	SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low;				//SPI极性，选择低极性
	SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;			//SPI相位，选择第一个时钟边沿采样，极性和相位决定选择SPI模式0
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;				//NSS，选择由软件控制
	SPI_InitStructure.SPI_CRCPolynomial = 7;				//CRC多项式，暂时用不到，给默认值7
